...
function renderDebts(){
  ...
  el.debtList.innerHTML=sorted.map(item=>{const balance=Math.max(0,item.total-item.paid),progress=item.total?Math.min(100,item.paid/item.total*100):0,installments=Math.max(0,Number(item.installments)||0);return`<article class="debt-item"><div class="creditor-cell"><strong title="${safe(item.creditor)}">${safe(item.creditor)}</strong><span class="priority-badge priority-${item.priority}">${priorityText[item.priority]}</span></div><div class="debt-value"><span>Valor total</span><strong>${currency.format(item.total)}</strong></div><div class="debt-value"><span>Valor pago</span><strong>${currency.format(item.paid)}</strong></div><div class="debt-value remaining"><span>Saldo restante</span><strong>${currency.format(balance)}</strong></div><div class="debt-value"><span>Parcelas restantes</span><strong>${installments||'—'}</strong></div><div class="debt-progress-cell"><div class="debt-progress-track"><div class="debt-progress-fill" style="width:${progress}%"></div></div><small>${progress.toLocaleString('pt-BR',{maximumFractionDigits:1})}% pago</small></div><button class="delete-button" type="button" data-delete-debt="${item.id}" title="Excluir dívida" aria-label="Excluir dívida de ${safe(item.creditor)}">×</button></article>`}).join('');
